...
select u.usuarioId, cp.chatId
from usuario u 
left join chatParticipante cp on cp.usuarioParticipante = u.usuarioId
where not exists(select cp2.chatId 
				 from chatParticipante cp2 
				 where cp2.chatId != cp.chatId 
					and cp2.chatId in (select cp4.chatId from chatParticipante cp4 where cp4.usuarioParticipante = cp.usuarioParticipante)
				 group by cp2.chatId 
				 having count(*) > (select count(*) from chatParticipante cp3 where cp3.chatId = cp.chatId) 
				 )
